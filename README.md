...
Opening a local `.sb` file offers **Install Module** in its details when durable writable module storage is available. The existing `sb next-boot add` operation copies it to the Next Boot set; installation does not change the running session.
